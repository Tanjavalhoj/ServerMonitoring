...
//-------------------------------- Server 1 ------------------------------------
        ...
        ssh2_auth_password($connection_server_1, '', '');
...
//        }
        ...
        $pattern = "/([\/a-z]+)[ ]+([0-9\.(A-Z)]+)+[ ]+([0-9\.(A-Z)]+)+[ ]+([0-9\.(A-Z)]+)+[ ]+([0-9%]+)+[ ]+([\/a-z]+)/";

        if (preg_match_all($pattern, $output_dfih_server_1, $matches_dfih)) {
...
//        }
        ...
        if (preg_match_all($pattern, $output_dfh_server_1, $matches_dfh)) {
...
//Ctreate table for dfih
        $createtable_dfih = "CREATE TABLE IF NOT EXISTS ssh_server_1_dfih(id_server_1 INT NOT NULL PRIMARY KEY AUTO_INCREMENT,"
        ...
        if ($conn->query($createtable_dfih) === TRUE) {
        ...
        $sliced = array_slice($matches_dfih, 1);
        //var_dump($sliced);
        $count_array = count($matches_dfih[0]);

        //  var_dump("count" . $count_array);
        $i_dfih = 0;
        while ($i_dfih < $count_array) {
            
            $sql_dfih = "INSERT INTO ssh_server_1_dfih (`Filesystem`,`Inodes`,`IUsed`,`IFree`,`IUsed_Procent`,`Mounted_on`)"
...
//Cteate table for dfh
        $createtable_dfh = "CREATE TABLE IF NOT EXISTS ssh_server_1_dfh(id_server_1 INT NOT NULL PRIMARY KEY AUTO_INCREMENT,"
                . "Filesystem VARCHAR(30) NOT NULL,"
...
// Data bliver insat i ssh_dfh databasen
...
//var_dump("count" . $count_array);
        ...
        while ($i_dfh < $count_array_dfh) {
            ...
            $sql_dfh = "INSERT INTO ssh_server_1_dfh (`Filesystem`,`Size`,`Used`,`Avail`,`Used_Procent`,`Mounted_on`)"
